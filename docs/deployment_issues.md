...
### 9. character_equipment item_id FK가 PostgreSQL에서 실제 적용됨

**증상**
```
sqlalchemy.exc.IntegrityError: INSERT INTO character_equipment ... RETURNING character_equipment.id
ForeignKeyViolation: insert or update on table "character_equipment" violates foreign key constraint
```

**원인**
초기 migration에 `character_equipment.item_id → items.id` FK가 정의되어 있음. SQLite는 FK를 기본적으로 무시하지만 PostgreSQL은 엄격하게 적용. 캐릭터 장비에 현재 패치 `items` 테이블에 없는 구버전 아이템이 포함되면 INSERT 실패. Migration 004 주석에 "FK는 DB에 원래부터 없었음"이라 적혀 있었으나 이는 SQLite 기준.

**해결**
Migration 007 추가: `character_equipment_item_id_fkey` FK 제거.

---

### 10. is_active 비교에 정수형 사용 — PostgreSQL boolean 타입 불일치

**증상**
```
psycopg.errors.UndefinedFunction: operator does not exist: boolean = integer
LINE 15: AND i.is_active = 1
```

**원인**
`roadmap_repo.py`의 raw SQL 쿼리에서 `is_active = 1` 사용. SQLite는 boolean을 정수로 저장하므로 동작하지만, PostgreSQL은 `boolean` 타입에 `= 1` 연산자가 정의되지 않음.

**해결**
`is_active = 1` → `is_active = true` 로 4곳 모두 수정.

---

### 11. CORS 미들웨어가 uncaught 예외 시 헤더 누락

**증상**
DB 에러(500)가 발생했을 때 브라우저에서 CORS 에러로 표시됨.

**원인**
FastAPI의 CORSMiddleware는 HTTPException에는 CORS 헤더를 추가하지만, 라우터에서 uncaught 예외가 발생하면 ASGI 레벨에서 연결이 끊어져 CORS 헤더가 포함되지 않음.

**해결**
`upsert_equipment` 호출을 try-except로 감싸고 `HTTPException(500)`으로 변환.
