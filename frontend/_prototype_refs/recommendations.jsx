// ── Screen 3: Recommendations — transparency dashboard ──
const { useState: useStateR, useMemo: useMemoR } = React;

// ── Sticky tradeoff legend ──
    ...
    <div className="weights" style={{ marginBottom: 14 }}>
      <div className="weights-h">
        ...
        <window.Prov
          ...
          sampleN={D.META.sampleN} />
      </div>
      <div className="gs-summary" style={{ marginBottom: 0, padding: 0, background: 'none', border: 'none' }}>
        <div className="gs-cell"><div className="l">Item Lvl</div><div className="v">{D.CHAR.ilvl.toFixed(1)}</div></div>
...
function RecommendationsScreen() {
  ...
  const [weights, setWeights] = window.useWeights();
  const [sortKey, setSortKey] = useStateR('_score');
  const [sortDir, setSortDir] = useStateR('desc');

  const D = window.GG_DATA;

  const rows = useMemoR(() => {
    const base = tab === 'raid' ? D.RAID.bosses : D.DUNGEONS;
    const scored = window.computeScores(base, weights);
  ...
  function onSort(key) {
    if (sortKey === key) {
      ...
      setSortKey(key);
      // time defaults to ascending (less is better)
    ...
    <div className="page-enter" style={{ maxWidth: 1320, margin: '0 auto', padding: '0 24px' }}>
      ...
      <div style={{ display: 'flex', alignItems: 'baseline', justifyContent: 'space-between', margin: '8px 0 6px' }}>
        ...
        <span style={{
          ...
          n = {D.META.sampleN.toLocaleString()} · {D.META.basis}
        </span>
      </div>
      <p style={{
      ...
      <div className="recs-grid" style={{ display: 'grid', gridTemplateColumns: 'minmax(0, 1fr) 280px', gap: 22 }}>
        ...
        <aside>
          <window.WeightControls
            weights={weights}
            onChange={setWeights}
            onReset={() => setWeights(window.WEIGHT_DEFAULTS)}
          />
          <GapSnapshot />
          <SourcesPanel />
        </aside>
